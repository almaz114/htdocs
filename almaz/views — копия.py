...
def Kovach(request):
    ...
    if request.method == "POST":
        today_time = datetime.today().strftime("%d-%m-%Y-%H:%M")  # получим тек время
        dict_base["last_update"] = str(today_time)  # add date/time to dict
        ...
        type_order = request.POST.get("type_order")     # получаем тип ордера
        open_price_stop = request.POST.get("open_price_stop")   # получаем цену соот-го ордера
        open_price_limit = request.POST.get("open_price_limit")   # получаем цену соот-го ордера
        stop_loss_price = request.POST.get("stop_loss_price")
        take_profit_price = request.POST.get("take_profit_price")
        cascade_orders = request.POST.get("cascade_orders")
        active = request.POST.get("active")

        # -- A D D/ U P D A T E  S I G N A L  T O  J S O N  F I L E
        if active == "Алгоритм Включен":
            dict_base["active"] = 1
            if symbol and len(symbol) > 1:
                dict_symbol = dict_base[symbol]
                # -- 1. режим открытия позиции
                if regime == "Открытие позиции":
